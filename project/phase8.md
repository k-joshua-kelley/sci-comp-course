...
Update your `CMakeLists.txt` to create `wavesolve_gpu`; as of this writing, CMake doesn't have infrastructure in place for GPU compilation with `nvc++`, so you'll have to look to the [example code](https://github.com/BYUHPC/sci-comp-course-example-cxx/blob/main/CMakeLists.txt) to see how to do so manually. Develop in a branch named `phase8` or tag the commit you'd like me to grade from `phase8`, and push it.
